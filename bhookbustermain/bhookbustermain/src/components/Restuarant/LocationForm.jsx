...
const RestaurantRegistration = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        setLoading(true);
        // FIXED: Added full URL for categories endpoint
        const response = await axios.get('https://bhookbuster-backend-3.onrender.com/category/getallcategories');
        if (response.data && response.data.data) {
          setCategories(response.data.data);
        }
        setLoading(false);
  ...
  const onSubmit = async (data) => {
    try { 
      ...
      setIsLoading(true);
      // FIXED: Added full URL for location endpoint
      const response = await axios.post('https://bhookbuster-backend-3.onrender.com/location/locationwithfile', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
